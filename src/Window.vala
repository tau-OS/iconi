...
public class IconMakerWindow : He.ApplicationWindow {
	...
	private const string ICON_GRID_DARK = "grid-dark";
	private const string ICON_GRID_LIGHT = "grid-light";
	...
	private void setup_ui () {
		...
		left_box.append (add_overlay_btn);
		...
		grid_preview_picture = create_icon_picture (ICON_GRID_DARK_MINI);
		...
		bool grid_prev_state = updating_properties;
		...
		updating_properties = grid_prev_state;
		update_grid_preview_icon ();
		update_grid_controls_sensitivity ();
		mappbar.append_menu (grid_menu_btn);
		...
		uint initial_variant_index = get_variant_index (model.icon_background_variant);
		bg_variant_drop.set_selected (initial_variant_index);
		...
		bg_variant_drop.notify.connect ((pspec) => {
			if (updating_properties)return;
			if (pspec.name != "selected")return;
			uint idx = bg_variant_drop.get_selected ();
			int variant_idx = (int) idx;
			if (variant_idx < 0 || variant_idx >= bg_variant_values.length)return;
			IconBackgroundVariant variant = bg_variant_values[variant_idx];
		...
		grid_overlay_switch.state_set.connect ((w, state) => {
			if (updating_properties)return false;
			model.show_grid_overlay = state;
			update_grid_controls_sensitivity ();
			canvas.queue_draw ();
			return false;
		});
		grid_variant_drop.notify.connect ((pspec) => {
			if (pspec.name != "selected")return;
			if (updating_properties)return;
		...
		export_btn.clicked.connect (() => {
			...
			file_dialog.set_initial_name (model.name);
	...
	private void update_grid_preview_icon () {
		...
		string icon_name = model.grid_overlay_variant == GridOverlayVariant.DARK ? ICON_GRID_DARK_MINI : ICON_GRID_DARK_MINI;
		grid_preview_picture.icon_name = icon_name;
	}
	...
	private void sync_background_variant_controls () {
		if (bg_variant_drop == null)return;
		bool previous = updating_properties;
		updating_properties = true;
		...
		if (bg_canvas_bg_row != null) {
			bg_canvas_bg_row.set_visible (custom);
		}
		if (bg_fill_mode_row != null) {
	...
	private void export_to_svg (string filename) {
		try {
			...
			data_stream.put_string ("  <rect id=\"background\" x=\"%g\" y=\"%g\" width=\"%g\" height=\"%g\" rx=\"%g\" ".printf (offset, offset, svg_size, svg_size, radius));
	...
	private void write_overlay_svg (GLib.DataOutputStream stream, string overlay_id, string resource_path, double scale, double offset_x, double offset_y) {
	...
	private void write_dev_badge_svg (GLib.DataOutputStream stream) {
